            ...
            <div class="grid-container">
                ...
                <div class="field1">
                    <div class="form-group">
                        ...
                        <QuillEditor toolbar="essential" v-model:content="this.tableData[0].sambutan" theme="snow"
                            content-type="html" />
                    </div>
                </div>
...
export default {
    ...
    methods: {
        fetchData() {
            axios
                .get(`http://localhost:8080/pemerintah/sambutan/1`)
                .then(({ data }) => {
        ...
        async updateData() {
            ...
            if (result.isConfirmed) {
                axios
                    .put("http://localhost:8080/pemerintah/sambutan/setting", {
                        sambutan: this.tableData[0].sambutan,
                        id_desa: this.id_desa,
                    })
                    .then((res) => {
                        if (res.data.status) {
                            Swal.fire("Data berhasil diubah.", res.data.message, "success");
                            this.$router.push('/pemerintahan/sambutan-kades');
                        } else {
